...
export default function DynamicPriceBox({ selectedPlan }: { selectedPlan: string }) {
  ...
  return (
    <div className="border rounded-md p-4 bg-white shadow-sm mt-4">
      <h2 className="text-lg font-bold">프리미엄 요금제</h2>
      <p className="text-2xl font-semibold mt-2">₩{price.toLocaleString()}원</p>

      {loading && (
        <p className="text-blue-500 text-sm mt-1">⏳ AI 최적 요금 분석 중...</p>
      )}
